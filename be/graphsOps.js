...
function getValue(results, i, date) {
  ...
  currentDate.setJSTHours(currentDate.getJSTHours() + i);
...
function getCommandStart(date, startX, startY, endX, endY) {
  ...
  var formatedDate = date.getJSTDate() + '/' + (date.getJSTMonth() + 1) + '/';
  formatedDate += date.getJSTFullYear();
...
exports.generate = function(date, callback) {
  ...
  maxDate.setJSTDate(maxDate.getJSTDate() + 1);
  ...
  stats.aggregate([ {
    ...
    if (error) {
      ...
      if (verbose) {
        console.log('Generating graph for ' + date.toJSTString());
      }
