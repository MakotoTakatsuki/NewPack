  ...
  zlib.gzip(task.data, function gotCompressedData(error, compressedData) {
    ...
    if (error) {
      ...
      referenceBlock[task.dest] = {
        ...
        lastModified : new Date().toJSTString()
      };
...
exports.processReadFile = function(finalPath, data, stats, task, callback) {
  ...
  var file = {
    lastModified : stats.mtime.toJSTString(),
    mime : mime,
    path : finalPath,
    content : data,
    compressed : false,
...
exports.addHeaderBoilerPlate = function(header, stats, isStatic) {
  ...
  if (canCache) {
    expiration.setJSTMinutes(expiration.getJSTMinutes() + staticExpiration);
  }
  ...
  if (useCacheControl) {
    ...
    header.push([ 'expires', expiration.toJSTString() ]);
  }
